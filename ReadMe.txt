...
3. Установить и использовать в проекте .NET Framework 4.6.1;
3. При выходе из программы ее настройки должны сохраняться,
   и эти же настройки должны загружаться при следующем запуске;
4. При выборе пункта 'Alarms' должен загружаться список будильников, 
   и должна быть возможность редактировать этот список;

TODO:
0. Сделать COMMIT и создать ветку 'FontHW';
1. Применить к часам кастомный шрифт;
2. При установке галочки 'Autorun' в контекстном меню, пограмма должна запускаться при старте системы;
3. Обеспечить перемещение часов по экрану, когда скрыты элементы управления;

DONE:
1. Почитать про двойную буферизацию;
2. Создать ветку 'ContextMenuTask1';
3. Когда установлена галочка 'Topmost', часы должны отображаться поверх остальных окон;
4. При двойном щелчке по иконке в области уведомлений (System tray) часы должны появляться на переднем плане;
5. При установке соответсвующих галочек в контекстном меню должны оторажаться дата и день недели;
6. Часы должны закрываться при нажатии на пункт меню 'Exit';
7. Пункты 'Foreground color' и 'Background color' должны менять цвет текста и фона на часах;
